...
3. 원본 DB와 첨부파일 목록의 백업을 만든다.
4. 새 임시 DB에 스키마를 생성한다.
5. 업무, 업무일지, 메모, 첨부파일 순으로 변환한다.
6. 개수, 외래키, 날짜, 파일 존재 여부를 검증한다.
7. 사용자에게 변환 결과와 경고를 보여준다.
8. 성공 시 임시 DB를 정식 v3 DB로 원자적으로 교체한다.
...
| deadline | ends_at 또는 별도 경고 | 값의 실제 사용 패턴을 fixture로 조사 후 확정 |
