...
- Phase 5B의 알림 서비스는 규칙 계산과 발송 이력 선점을 담당하고, Presentation 계층은
  반환된 알림 묶음만 표시한다.
- 각 알림 규칙의 다음 알림 시각과 반복 발생 시각을 DB에 캐시한다. 일정·반복·상태가
  바뀌면 트리거가 캐시를 무효화하며, 폴링은 인덱스로 현재 시각 이전의 규칙만 조회한다.
...
오늘 화면과 업무일지 검색 결과는 각각 필요한 페이지 범위만 조회한다. 월간 캘린더는
42일의 정확한 일정 개수를 DB에서 집계하되 미리보기는 최대 60개만 읽는다. 120개를 넘는
달은 개수 중심 요약 모드로 표시하고, 날짜를 선택할 때 해당 하루의 전체 일정만 별도로
조회한다. 반복 일정 역시 현재 보이는 월 또는 선택한 하루 범위에서만 전개한다.
알림 폴링은 전체 활성 규칙을 매번 전개하지 않고 `reminders(enabled, next_fire_at)` 인덱스로
도래한 규칙만 읽는다. 마이그레이션 직후 또는 일정 변경으로 캐시가 무효화된 규칙만
100개 단위로 초기화한다.
...
- 확인·완료·대기로 처리된 지 180일이 지난 알림 이력은 데이터 화면에 정리 후보로만
  표시하며 이 단계에서는 자동 삭제하지 않는다.
